...
_legendkws =dict(loc='lower left', fontsize=8, labelcolor='linecolor', frameon=False, mode='expand', ncol=3)
...
_keybindings = {
    '0': ('selfunc','None'),
    '1': ('selfunc','Add Luminosity'),
    '2': ('selfunc','Remove Luminosity'),
    '3': ('selfunc','Add IDPX'),
    '4': ('selfunc','Remove IDPX'),
    'q': ('morphfunc','OPEN'),
    'w': ('morphfunc','ERODE'),
    'e': ('morphfunc','CLOSE'),
    'r': ('morphfunc','BLACKHAT'),
    't': ('morphfunc','DILATE'),
    'y': ('morphfunc','TOPHAT'),
    'u': ('morphfunc','GRADIENT'),
    'i': ('morphfunc','HITMISS'),
    'a': ('retrfunc','EXTERNAL'),
    's': ('retrfunc','LIST'),
    'd': ('retrfunc','CCOMP'),
...
_selindexes = {'ACTION':['None','Add Luminosity','Remove Luminosity','Add IDPX','Remove IDPX',],
               'RETR':['EXTERNAL','LIST','CCOMP','TREE'],
                'CHAIN':['NONE','SIMPLE','TC89_L1','TC89_KCOS'],
                'MORPH':['CLOSE','OPEN','GRADIENT','TOPHAT','BLACKHAT','HITMISS','ERODE','DILATE']}
...
def pathfinder(fits_dir: fits.HDUList,saveloc=None,show_tooltips=True, headername='BOUNDARY',morphex=(cv2.MORPH_CLOSE,cv2.MORPH_OPEN), fcmode=cv2.RETR_EXTERNAL, fcmethod=cv2.CHAIN_APPROX_SIMPLE, cvh=False, ksize=5,**persists):
    ...
    with mpl.rc_context(rc={'font.family': 'FiraCode Nerd Font', 'axes.unicode_minus': False, 'toolbar': 'None', 'font.size': 8,
        ...
        global retattrs 
        global G_headername
        G_headername = headername
        global G_show_tooltips
        G_show_tooltips = show_tooltips
        retattrs = dict(LMIN =-np.inf, LMAX=np.inf, NUMPTS=np.nan, XYAREA_CTR=np.nan, XYAREA_CTRP=np.nan, XYAREA_IMG=imarea)
        ...
        fig = plt.figure()
        fw = [[fig.canvas.width(), int(600)],[fig.canvas.height(), 400]]
        fwm = [[fig.canvas.width(), 1920],[fig.canvas.height(), 1080]]
        if any(i[0] < i[1] for i in fw) or any(i[0] > i[1] for i in fwm):
        ...
        mgs = gs[1].subgridspec(12,12, wspace=0, hspace=0, height_ratios=[
        ...
        def on_key(event):
            if event.key in _keybindings:
                ...
                if action == 'selmode':
                    selradio.set_active(_selindexes['ACTION'].index(val))
                elif action == 'retrfunc':
                    retrbts.set_active(_selindexes['RETR'].index(val))
                elif action == 'morphfunc':
                    morphbts.set_active(_selindexes['MORPH'].index(val))
                elif action == 'chainfunc':
                    chainbts.set_active(_selindexes['CHAIN'].index(val))
                elif action == 'cvhfunc':
                    cvhbts.set_active(0)
                elif action == 'ksizeud':
...
'MORPH':[
    ...
    [      0,       1,     2,      3,         4,       5,         6,         7]],
'CHAIN':[
    [None,'NONE','SIMPLE','TC89_L1','TC89_KCOS'],
    [   0,    1,       2,         3,          4]]
