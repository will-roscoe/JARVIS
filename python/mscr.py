...
from jarvis.utils import fpath, fitsheader, ensure_dir, make_filename
...
def mk_stripped_polar(fits_obj,path=None,**kwargs)->None:
    ...
    ####################################################################################################
    ## THIS bit should likely be replaced by something that can adapt better, see outputs.
...
for i in [f"{m:0>2}" for m in range(1,21)]:
    ...
    graddat = np.where(gradmap(coadded_data)<70,np.nan,gradmap(coadded_data))#gradmap(coadded_data)#
    ...
    img = mk_stripped_polar(proc,bg_color='black', facecolor='white', cmap=cmr.neutral_r,len_fit=len(fitsfiles)+5)
