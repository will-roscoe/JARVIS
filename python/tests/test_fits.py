...
import pytest
from jarvis.utils import fpath, fits_from_parent, fitsheader, prepare_fits, get_datetime, make_filename, update_history, debug_fitsheader, debug_fitsdata
...
def test_fits_from_parent():
    new_fits = fits_from_parent(fits_obj, new_data=np.ones_like(fits_obj[1].data))
    assert new_fits[1].data.shape == fits_obj[1].data.shape
def test_prepare_fits():
    new_fits = prepare_fits(fits_obj, regions=False, moonfp=False, fixed='lon', rlim=40,full=True, crop=1)
...
def test_make_filename():
    ...
    assert make_filename(prepared_fits)[0:len(expect)] == expect
