...
aperture = hdu_list[0].header[
    "FILTER"
]  # filter type - important as it determines the Gustin conversion factors for intensity/counts/powers etc.
...
print(1.0 / cts2kr)  # reciprocal of cts2kr to match in Gustin+2012 Table 1.
...
mid_ex_saturn = start_time_saturn + (exposure / 2.0)  # mid-exposure time at Saturn
mid_ex = start_time + (exposure / 2.0)  # mid-exposure time at HST
...
plt.pcolormesh(lons, lats, np.flip(np.flip(im_clean, axis=1)), cmap="cubehelix", vmin=0.0, vmax=1000.0)
...
plt.imshow(image_extract, cmap="cubehelix", vmin=0.0, vmax=1000.0)
...
ax.tick_params(axis="x", pad=-1.0)  # shift position of LT labels
...
plt.pcolormesh(theta, rho, image_extract, cmap="cubehelix", vmin=0.0, vmax=1000.0)

dr = [[i[0] for i in dark_region], [i[1] for i in dark_region]]
plt.plot([np.radians(360 - r) for r in dr[1]], dr[0], color="red", linewidth=3.0)
...
# Add colourbar: ---------------------------------------------------------------
cbar = plt.colorbar(ticks=[0.0, 100.0, 500.0, 900.0], pad=0.05)
...
plt.pcolormesh(testlons, testcolats, image_extract, cmap="cubehelix", vmin=0.0, vmax=1000.0)
plt.title("ROI")
plt.xlabel("SIII longitude [deg]")
plt.ylabel("co-latitude [deg]")
...
def _cylbroject(pimage, cml, dece, dmeq, xcen, ycen, psize, nppa, req, obt, ndiv=2, correct=True):
    ...
    plen = (psize / 3600.0) * deg2rad * dmeq * 1.49598e8
    # print(plen) # in km
...
def cylbroject(image, ndiv=2):
    ...
    bimage = _cylbroject(image, cml, dece, dist, pcx, pcy, pxsec, nppa, rpkm + delrpkm, oblt, ndiv, True)
    return bimage
